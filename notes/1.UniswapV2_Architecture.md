## Uniswap V2 Architecture: Introduction to Automated Market Makers

**Uniswap** is a DeFi app that enables traders to swap one token for another in a trustless manner. It was one of the early automated market makers for trading.

Automated market makers are an alternative to an [order book](https://www.investopedia.com/terms/o/order-book.asp), which the reader is assumed to already familiar with
...
This guarantees that the pool's asset holdings can only stay the same or increase. Most pools enforce some kind of fee. Not only should the product of the balances increase, but it should increase by at least a certain amount to account for a fee.

Assets are provided to the pool by _liquidity providers_, who receive so-called LP tokens to represent their share of the pool. Liquidity provider balances are tracked in a manner similar to how [ERC4626](https://rareskills.io/post/erc4626) works. The difference between an AMM and ERC 4626 is the ERC 4626 only supports one asset but an AMM has two tokens. Just like a vault, the liqudity providers' share of the pool stays the same, but the product $xy$ gets larger, so their slice is larger.

## Advantages of AMMs

### AMMs do not have a bid-ask spread

In an AMM, price discovery is automatic. It's determinne by the ratio of assets in the pool. Specifically, if we have token $x$ and token $y$, price is determined as follows:
...
There is no need to wait for a suitable "bid" or "ask" order to show up. It always exists. If there is a mismatch between the price in an AMM and another exchange, then a trader will arbitrage the difference, bringing the prices back into balance.
...
There are two major drawbacks to automated market makers.

1.  The price always moves
...
1. Victim's buy: drive up price even further
1. Attacker's sell: sell the first buy at a profit

### Liquidity providers don't have control over the price their assets are sold at

Liqudity providers can only provide assets proportional to the current ratio of tokens in the pool. For example, if there are 100 token $x$ and 200 token $y$, the new liquidity provider must provide twice as many token $y$ as $x$.

In a traditional order book, a market maker can place limit orders at levels they believe reflect a desirable bid for ask (For example, place a bid order below the current market price or place a sell order above the current market price), but this is not possible with an automated market maker. Remember that Automated Market Makers use a formula to set prices based on the asset ratios in the pool, as as result market makers cannot set spefific prices at which they wish to sell their assets.

### Liquidity providers for AMMs may suffer from Impermanent Loss

Let's say in hypothetical scenario Ether starts at $10 and becomes worth $1,000 later.
...
If they kept the money in an AMM, then they would have missed out on most of the gains. The AMM would have 0.1 ETH and 100 USD after the price change. This correctly prices ETH at $1,000, but the net value of the pool is less than $990. Here is the what the pool holdings will like before and after the price change.

$$
(\text{ETH}_{\text{before}})(\text{USD}_{\text{before}}) \leq (\text{ETH}_{\text{after}})(\text{USD}_{\text{after}}) \\
(1~\text{ETH})(10~\text{USD}) \leq (0.1~\text{ETH})(100~\text{USD}) \\
10 \leq 10
$$

Although the amount of stablecoins went up to 10x, the amount of Ether went down. The net result is that the value of our assets, when in the pool, increased less than if we'd held the assets separately.

Here is a table showing the relative performance of holding ETH and USD in a pool vs just holding them.

|        | Ether Pool Balance | Stablecoin Pool Balance | ETH x Stablecoin | $ value of 1 ETH | Value of assets if providing liquidity |          Value of just holding |
| :----- | :----------------: | :---------------------: | :--------------: | :--------------: | :------------------------------------: | -----------------------------: |
| Before |         1          |           10            |        10        |        10        |         $20 ($10 ETH + 10 USD)         |     $20 ($10 x 1 ETH + 10 USD) |
| After  |        0.1         |           100           |        10        |       1000       |       $200 ($100 ETH + 100 USD)        | $1010 ($1000 x 1 ETH + 10 USD) |
| Gain.  |                    |                         |                  |                  |                  $180                  |                           $990 |
...
The architecture of Uniswap V2 is surprisingly simple. At it's core is the **UniswapV2Pair** contract that holds the two ERC20 tokens that traders can swap against, or liquidity providers can provide liquidity for. Every different possible **UniswapV2Pair** contract doest not exist, a new one can be permissionlessly created from the **UniswapV2Factory** contract. **UniswapV2Pair** contracts are also ERC20 tokens (they inherit from ERC20), and that token is used to track deposits similar to how ERC 4626 works.
...
1. [Factory](https://github.com/Uniswap/v2-core/blob/master/contracts/UniswapV2Factory.sol)
1. [Pair (which inherits ERC20)](https://github.com/Uniswap/v2-core/blob/master/contracts/UniswapV2Pair.sol)
1. [Router](https://github.com/Uniswap/v2-periphery/tree/master/contracts)

### The core - periphery pattern

Observe that the router contract is in repository called "v2 periphery" and the pair is in the "v2 core" repository. Uniswap V2 follows the "core / periphery" design pattern where the most essential logic is held in the core while the "optional" logic is held in the periphery.
...
### How to locate a pool, given two addresses
...
The [EIP 1167 clone](https://rareskills.io/post/eip-1167-minimal-proxy-standard-with-initialization-clone-pattern) pattern is used to create a collection of similar contracts, so why not use that here? Although the deployment would be cheaper, it would introduce an extra 2,600 gas per transaction due to the [delegatecall](https://rareskills.io/post/delegatecall). Since pools are intended to be used frequently, the cost savings from deployment would eventually be lost after a few hundred transactions, so it is worth deploying a pool as a new contract.
