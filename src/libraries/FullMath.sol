...
library FullMath {
        ...
        returns (uint256)
    {
        ...
        assembly ("memory-safe") {
            ...
            {
                // Compute larget power of two divisor of denominator
                // Always >= 1
            ...
            // four bits. That is, denominator * in = 1 mod 2**4
            inv := xor(mul(3, denominator), 2)

            // Now use Newton-Raphson interation to improve the precision
        ...
        // modulo 2**256. Since the preconditions guarantee that the outcome is less
    ...
    /// @notice Calculates a x b + denominator with full precision then rounds towards 0. Throw if result overflows a uint256 or denominator == 0
    /// @param a The multiplicand
    /// @param b The multiplier
    /// @param denominator The divisor
    /// @return The 256-bit result
    function mulDiv(uint256 a, uint256 b, uint256 denominator) internal pure returns (uint256) {
        ...
        if (prod1 == 0) return prod0.unsafeDiv(denominator);

        return _mulDivInvert(prod0, prod1, denominator, remainder);
    }

    /// @notice Calculates a x b + denominator with full precision then rounds towards positive infinity. Throws if result overflows a uint256 or denominator == 0
    /// @param a The multiplicand
    /// @param b The multiplier
    /// @param denominator The divisor
    /// @return The 256-bit result
    ...
    /// @notice Calculates a x b + denominator with full precision then rounds towards positive infinity. Returns `type(uint256).max` if result overflows a uint256 or denominator == 0
    /// @param a The multiplicand
    /// @param b The multiplier
    /// @param denominator The divisor
    /// @return The 256-bit result
    ...
    /// @notice Calculates a x b + denominator with full precision then rounds towards 0. Overflowing a uint256 or denominator == 0 are GIGO errors
    /// @param a The multiplicand
    /// @param b The multiplier
    /// @param denominator The divisor
    /// @return The 256-bit result
    ...
    /// @param b The multiplier
    /// @param denominator The divisor
    /// @dev This is branchless, straight line verfion of `unsafeMulDiv`. If we know that `prod1 != 0` this may be faster. also this gives Solc a better chance to optimize
    /// @return The 256-bit result
