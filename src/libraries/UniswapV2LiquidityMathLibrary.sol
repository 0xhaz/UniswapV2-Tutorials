...
// library containing some math for dealing with the liquidity shares of a pair, e.g. computing their exact value in underlying tokens

library UniswapV2LiquidityMathLibrary {
    using SafeMath for uint256;

    // computes the direction and magnitude of the profit-maxmizing trade
    ...
    // gets the reserves after an arbitrage moves the price to the profit-maximizing ratio given observed true price
    function getReservesAfterArbitrage(
        ...
        require(reserveA > 0 && reserveB > 0, "UniswapV2Arbitrage: ZERO_PAIR_RESERVES");
    ...
    function computeLiquidityValue(
        ...
        if (feeOn && kLast > 0) {
            uint256 rootK = Math.sqrt(reservesA.mul(reservesB));
    ...
    // NOTE: this is subject to manipulation, e.g sandwich attacks. prefer to passing a manipulation resistant price tp
